    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-perms="'coupon:memberprice:delete'"
        >
          批量删除
        </el-button>
      </el-form-item>
      ...
      >
        <template #default="scope">
          <el-button
            ...
            v-perms="'coupon:memberprice:update'"
          >
            修改
          </el-button>
          <el-button
...
// 从商品管理「会员价格」跳转带入的 skuId（自动打开添加弹窗并预填）
const skuId = ref(Number(route.query.skuId) || 0)

const getDataList = async () => {
  dataListLoading.value = true
  try {
    const response = await reqMemberPriceList(
      pageIndex.value,
      pageSize.value,
      dataForm.value.key,
    )
...
const deleteHandle = async (id?: number) => {
  ...
  try {
    ...
    const response = await reqMemberPriceDelete(ids)
    if (response && response.code === 200) {
      clearSkuIdQuery()
      ElMessage({
        type: 'success',
...
onMounted(() => {
  getDataList()
  // 商品管理「会员价格」跳转：自动打开添加弹窗并预填 skuId
  if (skuId.value) {
    addOrUpdateVisible.value = true
    nextTick(() => {
