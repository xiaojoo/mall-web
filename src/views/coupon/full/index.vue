...
// 从商品管理「满减设置」跳转带入的 skuId（自动打开添加弹窗并预填）
const skuId = ref(Number(route.query.skuId) || 0)

const getDataList = async () => {
  dataListLoading.value = true
  try {
    const response = await reqSkuFullReductionList(
      pageIndex.value,
      pageSize.value,
      dataForm.value.key,
    )
...
const deleteHandle = async (id?: number) => {
  ...
  try {
    ...
    const response = await reqSkuFullReductionDelete(ids)
    if (response && response.code === 200) {
      clearSkuIdQuery()
      ElMessage({
        type: 'success',
...
onMounted(() => {
  getDataList()
  // 商品管理「满减设置」跳转：自动打开添加弹窗并预填 skuId
  if (skuId.value) {
    addOrUpdateVisible.value = true
    nextTick(() => {
