<template>
  <el-dialog
    ...
    :title="'批量添加' + (attrType === 0 ? '销售属性' : '规则参数')"
    width="720px"
...
每行一个{{
          attrType === 0 ? '销售属性' : '规则参数'
        }}，格式：属性名|可选值|所属分组ID|图标；也支持粘贴 JSON 数组：
[
  { "attrName": "属性名", "valueSelect": "红;蓝", "valueType": 1, "catelogId": 225, "attrGroupId": 1, "icon": "xxx" }
]
所属分类可多选，仅三级分类生效（每个生成一条）；JSON 里指定了 catelogId 则优先。</pre
    ...
    >
      ...
      <el-table-column label="属性ID" prop="attrId" width="160" />
      <el-table-column
        v-if="hasFailure"
        label="失败原因"
        prop="error"
...
import { reqAttrBatchSave } from '@/api/product/baseattr/index.ts'
...
// 行格式：属性名 | 可选值 | 所属分组ID | 图标（可选值多个用;分隔）
  ...
  return text
    ...
    .map((line) => {
      ...
      return {
        attrName: parts[0],
        valueSelect: parts[1] || '',
        attrGroupId: parts[2] ? Number(parts[2]) || null : null,
        icon: parts[3] || '',
      }
    })
}
...
const doBatchAdd = async () => {
  ...
  try {
    ...
    list = list.flatMap((item: any) => {
      const base = {
        ...
        attrType: item.attrType ?? props.attrType,
        enable: item.enable ?? 1,
        showDesc: item.showDesc ?? 0,
        attrGroupId: item.attrGroupId ?? null,
      }
  ...
  try {
    const res: any = await reqAttrBatchSave(list)
    if (res && res.code === 200) {
      // 后端返回不含分类名，按序号合并回显
