...
import {
  reqSeckillPromotionDelete,
  reqSeckillPromotionList,
...
const getDataList = async () => {
  ...
  try {
    const response = await reqSeckillPromotionList(
      pageIndex.value,
      pageSize.value,
      dataForm.value.key,
    )
...
const deleteHandle = async (id?: number) => {
  ...
  try {
    ...
    const response = await reqSeckillPromotionDelete(ids)
    if (response && response.code === 200) {
      ElMessage({
        type: 'success',
        message: '秒杀活动删除成功',
