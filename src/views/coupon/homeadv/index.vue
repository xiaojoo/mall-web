...
import { reqHomeDdvDelete, reqHomeDdvList } from '@/api/coupon/homeadv'
...
const getDataList = async () => {
  ...
  try {
    const response = await reqHomeDdvList(
      pageIndex.value,
      pageSize.value,
      dataForm.value.key,
    )
...
const deleteHandle = async (id?: number) => {
  ...
  try {
    ...
    const response = await reqHomeDdvDelete(ids)
    if (response && response.code === 200) {
      ElMessage({
        type: 'success',
        message: '首页广告删除成功',
