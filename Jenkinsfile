...
pipeline {
    ...
    stages {
        ...
        stage('构建 dist (node 容器)') {
            steps {
                sh "docker run --rm --network host -v \"\$PWD\":/app -w /app -u \"\$(id -u):\$(id -g)\" -e HOME=/tmp -e NPM_CONFIG_REGISTRY=https://registry.npmmirror.com node:22-alpine sh -c 'npm install -g --prefix /tmp/pnpm-global pnpm@9.15.0 >/dev/null 2>&1 && export PATH=/tmp/pnpm-global/bin:\$PATH && pnpm install --frozen-lockfile && pnpm build:${params.BUILD_MODE}'"
