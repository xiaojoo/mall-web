...
import { reqWareDelete, reqWareInfoList } from '@/api/ware/wareinfo'
...
const getDataList = async () => {
  ...
  try {
    const response = await reqWareInfoList(
      pageIndex.value,
      pageSize.value,
      dataForm.value.key,
    )
...
const deleteHandle = async (id?: number) => {
  ...
  try {
    ...
    const response = await reqWareDelete(ids)
    if (response && response.code === 200) {
      ElMessage({
        type: 'success',
        message: '仓库删除成功',
